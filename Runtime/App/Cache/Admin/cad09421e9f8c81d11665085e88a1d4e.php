...
<form id="pagerForm" action="/ehview/index.php/Admin/Video" method="post">
        ...
        <a href="<?php echo U('add');?>" rel="videoadd"  target="navtab" title="新增视频"  class="btn btn-green btn-sm">新增</a>
        </div>
        ...
        <thead>
            <tr>
                <th width="40">ID</th>
				 ...
				 <th class="orderby <?php if(($order) == "context"): echo ($sortImg); endif; ?>" orderField="context">视频内容</th>
				 <th  width="120" class="orderby <?php if(($order) == "status"): echo ($sortImg); endif; ?>" orderField="status">状态</th>
                <th width="220">操作</th>
            </tr>
        </thead>
        <tbody>
           <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				...
				<td>
                    <a href="/ehview/index.php/Admin/Video/edit/id/<?php echo ($vo['id']); ?>/navTabId/<?php echo CONTROLLER_NAME;?>" rel="videoedit<?php echo ($vo["id"]); ?>"  class="btn btn-green btn-sm" target="navtab" title="编辑<?php echo ($vo["title"]); ?>" >编辑</a>
                   <?php echo (showstatus($vo['status'],$vo['id'],CONTROLLER_NAME)); ?>
                    <a href="/ehview/index.php/Admin/Video/foreverdelete/id/<?php echo ($vo['id']); ?>/navTabId/<?php echo CONTROLLER_NAME;?>" class="btn btn-red btn-sm" target="ajaxTodo" title="确定要删除该行信息吗？">删</a>
                </td>
