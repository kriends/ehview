<?php if (!defined('THINK_PATH')) exit();?><div class="pageContent">
    <form action="/ehview/index.php/Admin/Role/update/navTabId/<?php echo CONTROLLER_NAME;?>" id="j_custom_form" class="pageForm form-validate" method="post" callback="dialogAjaxDone" noEnter>
     
	  <input type="hidden" name="id" value="<?php echo ($vo["id"]); ?>" />
        <div class="pageFormContent form-horizontal" layouth="60">
            <div class="form-group">
                <label for="j_name" class="control-label col-sm-2">组名：</label>
                <input type="text" class="form-control validate[required,custom[chinese]] required" size="30" name="name" id="j_name" value="<?php echo ($vo["name"]); ?>"  >
            </div>
            ...
            <div class="form-group">
				...
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><option value="<?php echo ($list["id"]); ?>" <?php if(($list['id']) == $vo['pid']): ?>selected<?php endif; ?>><?php echo ($list["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                 </select>
            ...
            <div class="form-group">
                ...
                <textarea name="remark" id="j_remark" class="form-control autosize" rows="4" cols="30"><?php echo ($vo["remark"]); ?></textarea>
            </div>
		</div>
		<div class="formBar">
            <ul>
