...
<form id="pagerForm" action="/ehview/index.php/Admin/Role" method="post">
    ...
    <form onsubmit="return navTabSearch(this);" class="form-inline"  action="/ehview/index.php/Admin/Role" method="post">
       ...
       <a href="/ehview/index.php/Admin/Role/add" target="dialog" rel="roleadd" mask="true"  width="500" height="400" class="btn btn-green btn-sm">新增</a>
       </div>        
                    <div class="btn-group pull-right">
                        ...
                        <ul class="dropdown-menu right" role="menu">
                            ...
                            <li><a href="/ehview/index.php/Admin/Role/selectedDelete"  title="确定要删除选中信息吗？" target="checkedAjaxTodo" idname="ids">删除选中</a></li>
                        </ul>
                    </div>
        ...
        <tbody>
           ...
           <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr> <td></td>
					...
					<td>
                                            ...
                                            <a href="/ehview/index.php/Admin/Role/access/groupId/<?php echo ($vo['id']); ?>" rel="roleaccess" target="dialog" mask="true"  class="btn btn-green btn-sm" title="<?php echo ($vo['name']); ?> 权限设置 " width="800" height="600">授权 </a>
                                            <a href="/ehview/index.php/Admin/Role/user/id/<?php echo ($vo['id']); ?>/navTabId/<?php echo CONTROLLER_NAME;?>" rel="roleuser" class="btn btn-green btn-sm" target="dialog" mask="true" title="<?php echo ($vo['name']); ?> 用户列表 ">用户列表</a>
                                            <a href="/ehview/index.php/Admin/Role/edit/id/<?php echo ($vo['id']); ?>/navTabId/<?php echo CONTROLLER_NAME;?>" rel="roleedit<?php echo ($vo['id']); ?>" class="btn btn-green btn-sm" target="dialog"  mask="true"  width="500" height="400" >编辑</a>           
                                             <?php echo (showstatus($vo['status'],$vo['id'])); ?>
                                            <a href="/ehview/index.php/Admin/Role/foreverdelete/id/<?php echo ($vo['id']); ?>/navTabId/<?php echo CONTROLLER_NAME;?>" class="btn btn-red btn-sm" target="ajaxTodo" title="确定要删除该行信息吗？">删</a>
                                        </td>
