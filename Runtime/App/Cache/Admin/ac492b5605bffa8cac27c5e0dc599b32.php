...
<form id="pagerForm" action="/ehview/index.php/Admin/Article" method="post">
        ...
        <a href="<?php echo U('add');?>" rel="articleadd"  target="navtab" title="新增文章"  class="btn btn-green btn-sm">新建文章</a>
        </div>
    ...
    <table class="j-table" width="100%" layoutH="95">
        <thead>
            <tr>
                ...
                <th  width="120" class="orderby <?php if(($order) == "status"): echo ($sortImg); endif; ?>" orderField="status">文章状态</th>
                <th width="200">操作</th>
            </tr>
        </thead>
        <tbody>
           <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				...
				<td>
                    <a href="/ehview/index.php/Admin/Article/edit/id/<?php echo ($vo['id']); ?>/navTabId/<?php echo CONTROLLER_NAME;?>" rel="articleedit<?php echo ($vo["id"]); ?>"  class="btn btn-green btn-sm" target="navtab" title="编辑<?php echo ($vo["title"]); ?>" >编辑</a>
                   <?php echo (showstatus($vo['status'],$vo['id'],CONTROLLER_NAME)); ?>
                    <a href="/ehview/index.php/Admin/Article/foreverdelete/id/<?php echo ($vo['id']); ?>/navTabId/<?php echo CONTROLLER_NAME;?>" class="btn btn-red btn-sm" target="ajaxTodo" title="确定要删除该行信息吗？">删</a>
                </td>
